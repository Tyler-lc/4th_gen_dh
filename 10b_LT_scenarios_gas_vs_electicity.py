...
for rows, columns in tqdm(df_combinations.iterrows(), total=len(df_combinations)):
    ...
    df_npv.to_csv(
        sensitivity_results_dir(simulation, analysis_type) / "data" / f"{analysis_type}_gas{gas_multiplier}_el{electricity_multiplier}.csv"
    )
...
def create_savings_scatter_plot(all_npv_data, df_combinations):
    ...
    # Save plot
    plt.savefig(
        sensitivity_results_dir(simulation, analysis_type) / "plots" / "price_sensitivity_scatter.png",
        bbox_inches="tight",
        dpi=300,
    )
    plt.close()
...
def create_savings_heatmap(all_npv_data, df_combinations):
    ...
    plt.tight_layout()
    plt.savefig(
        sensitivity_results_dir(simulation, analysis_type) / "plots" / "price_sensitivity_heatmap.png",
        bbox_inches="tight",
        dpi=300,
    )
    plt.close()
...
def create_savings_contour(all_npv_data, df_combinations):
    ...
    plt.tight_layout()
    plt.savefig(
        sensitivity_results_dir(simulation, analysis_type) / "plots" / "price_sensitivity_contour.png",
        bbox_inches="tight",
        dpi=300,
    )
    plt.close()
...
def export_mfh_data(all_npv_data, df_combinations, simulation, analysis_type):
    ...
    os.makedirs(
        sensitivity_results_dir(simulation, analysis_type) / "data", exist_ok=True
    )
    # Export to CSV
    results_df.to_csv(
        sensitivity_results_dir(simulation, analysis_type) / "data" / "mfh_savings_analysis.csv",
        index=False,
    )
    return results_df
...
def create_mfh_contour(all_npv_data, df_combinations):
    ...
    plt.tight_layout()
    plt.savefig(
        sensitivity_results_dir(simulation, analysis_type) / "plots" / "mfh_price_sensitivity_contour.png",
        bbox_inches="tight",
        dpi=300,
    )
    plt.close()

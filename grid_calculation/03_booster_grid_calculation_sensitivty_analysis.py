...
for flow_temp in grid_temperatures:
    ####### Define supply nodes
    n_supply_list = [
    ...
    # runs — these blocks run per sensitivity iteration, so the cost compounds.
    # Flip the guard to True when investigating geometry issues.
    if False:
    ...
    road_nw = ox.graph_from_polygon(
        polygon_wgs84, simplify=False, custom_filter=high_cf
    )
    ...
    # pipeline runs. Supply and demand loops call plt.plot() per node, so
    # this block compounds sharply across sensitivity iterations.
    if False:
        fig, ax = plt.subplots(figsize=(10, 10))
        ox.plot_graph(
    ...
    # slowest step when enabled, and here it runs once per sensitivity
    # iteration. Flip the guard to True for a one-off topology check.
    if False:
    ...
    for node_id, node_data in n_supply_dict.items():
        folium.Marker(
            ...
            popup=folium.Popup(
                f"Supply Node {node_id}<br>Capacity: {node_data['cap']} MW",
                max_width=300,
            ),
    ...
    for node_id, node_data in n_demand_dict.items():
        folium.Marker(
            location=node_data["coords"],
            popup=folium.Popup(
                f"Demand Node {node_id}<br>Capacity: {node_data['cap']} MW",
                max_width=300,
            ),

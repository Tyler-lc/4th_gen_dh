...
# areas_demand includes buildings with NFA < 30 that are filtered out of billing.
# Grid losses are from EMBERS (physical grid property, independent of building filter).
...
lcoh_heat_grid_boosters = calculate_future_values(
    {"heat from grid": (LCOH_HP + LCOH_dhg) * yearly_heat_supplied_large_hp * 1000}, n_years_hp
)
...
### DH operator revenue excludes VAT — VAT is collected on behalf of government
...
plt.savefig(
    PLOTS_DIR
    / "booster"
    / f"booster_AverageSavings_reduction_factor_{reduction_factor}_dhg_lifetime_{dhg_lifetime}.png"
)
plt.close()
...
plt.savefig(
    PLOTS_DIR
    / "booster"
    / f"booster_SavingsAverage_NFA_reduction_factor_{reduction_factor}_dhg_lifetime_{dhg_lifetime}.png"
)
plt.close()
n_columns = 3
...
plt.savefig(
    PLOTS_DIR
    / "booster"
    / f"booster_SavingsDistribution_reduction_factor_{reduction_factor}_dhg_lifetime_{dhg_lifetime}.png"
)
plt.close()
...
plt.savefig(
    PLOTS_DIR
    / "booster"
    / f"booster_SavingsDistribution_NFA_reduction_factor_{reduction_factor}_dhg_lifetime_{dhg_lifetime}.png"
)
plt.close()
...
plt.savefig(
    PLOTS_DIR
    / "booster"
    / f"booster_EnergySavingsVsNFA_reduction_factor_{reduction_factor}_dhg_lifetime_{dhg_lifetime}.png"
)
plt.close()
...
parameters = {
    ...
    "total_electricity_boosters_kwh": float(
        areas_demand["area total boosters demand [kWh]"].sum()
    ),
    "SCOP_lshp": float(

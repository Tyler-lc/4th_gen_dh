...
def sensitivity_analysis(
    ...
    # we are going to test the system with constant COP
    cop_hourly = pd.Series(max_COP, index=outside_temp.index)

    # max_cop = cop_hourly.max()
    # print(cop_hourly.head(200))
    cop_hourly.to_csv(f"max_cop_test/max_cop_{max_COP}.csv")
    ...
    print(f"initial_electricity_cost: {initial_electricity_cost}")
    ###################################################################################
    ###################################################################################
    ############################ Heat Pump LCOE Data  #################################
    ###################################################################################
...
for rows, columns in df_sensitivity_parameters.iterrows():
    ...
    os.makedirs(
        f"sensitivity_analysis/{simulation}/{analysis_type}_test", exist_ok=True
    )
    os.makedirs(
        f"sensitivity_analysis/{simulation}/{analysis_type}_test/plots",
        exist_ok=True,
    )
    os.makedirs(
        f"sensitivity_analysis/{simulation}/{analysis_type}_test/data",
        exist_ok=True,
    )
    ...
    for value in tqdm(values):
        ...
        df_npv.to_csv(
            f"sensitivity_analysis/{simulation}/{analysis_type}_test/data/{analysis_type}_{value}.csv"
        )
    ...
    plt.tight_layout()
    plt.savefig(
        f"sensitivity_analysis/{simulation}/{analysis_type}_test/plots/{analysis_type}_sensitivity_analysis.png"
    )
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(
        f"sensitivity_analysis/{simulation}/{analysis_type}_test/plots/{analysis_type}_average_savings_sensitivity.png",
        bbox_inches="tight",
    )
    plt.close()
    ...
    for value in values:
        ...
        plt.tight_layout()
        plt.savefig(
            f"sensitivity_analysis/{simulation}/{analysis_type}_test/plots/{analysis_type}_savings_distribution_{value}.png"
        )
        plt.close()
    ...
    if num_analysis == 4:  # max COP analysis
        ...
        # Save first figure
        fig1.savefig(
            f"sensitivity_analysis/{simulation}/{analysis_type}_test/plots/{analysis_type}_detailed_analysis_part1.png",
            dpi=100,
        )
        plt.close(fig1)
        ...
        # Save second figure
        fig2.savefig(
            f"sensitivity_analysis/{simulation}/{analysis_type}_test/plots/{analysis_type}_detailed_analysis_part2.png",
            bbox_inches="tight",
            dpi=100,
        )
        plt.close(fig2)

...
# for now we can keep it this way. But we need to run the other file first to use a more precise number
estimated_grid_losses = 0.1  # estimated grid losses are 10% of the delivered energy

areas_demand["final_energy_dh [kWh]"] = areas_demand["delivered_energy_dh"] * (
    1 + estimated_grid_losses
)  # this is the energy demand generated by the large scale heat pumps!


estimated_capacity = areas_demand[
    "final_energy_dh [kWh]"
].max()  # this is the total large scale heat pump capacity
...
heat_pump_load = (  # this is only used to calculate the O&M Var for the large scale heat pump
    areas_demand["final_energy_dh [kWh]"] / n_heat_pumps / 1000
...
P_el = (
    areas_demand["final_energy_dh [kWh]"] / cop_hourly
...
yearly_heat_supplied_large_hp = areas_demand["final_energy_dh [kWh]"].sum() / 1000  # MW
